        ...
        try {
          ...
          return {
            ...
            insert: function(c, e) {
              ...
              for (var d in e) {
                a +=
                  Object.keys(e)[Object.keys(e).length - 1] == d
                    ? '`' + d + '`'
                    : '`' + d + '`, ';
                b +=
                  Object.keys(e)[Object.keys(e).length - 1] == d ? '?' : '?, ';
            ...
            insertOrReplace: function(c, e) {
              var f =
                'INSERT OR REPLACE INTO `{tableName}` ({fields}) VALUES({values});';
              var a = '',
                b = '',
                v = [];
              for (var d in e) {
            ...
            whereClause: function(b) {
              ...
              for (var c in b) {
                if (
                  ...
                  typeof b[c]['value'] !== 'undefined' &&
                  typeof b[c] === 'object' &&
                  !b[c]['value'].match(/NULL/gi)
                )
                  v.push(b[c]['value']);
                a +=
                  typeof b[c] === 'object'
                    ? typeof b[c]['union'] === 'undefined'
                      ? typeof b[c]['value'] === 'string' &&
                        b[c]['value'].match(/NULL/gi)
                        ? '`' + c + '` ' + b[c]['value']
                        : '`' + c + '` ' + b[c]['operator'] + ' ? '
                      : typeof b[c]['value'] === 'string' &&
                        ...
                        : '`' +
                          ...
                          b[c]['union'] +
                          ' '
                    : typeof b[c] === 'string' && b[c].match(/NULL/gi)
                      ? '`' + c + '` ' + b[c]
                      : '`' + c + '`=?';
              }
              return {
